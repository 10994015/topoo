...
const pageTitle = computed(() => {
  return isEditMode.value ? '編輯單位標籤' : '新增單位標籤'
})
...
const validateForm = () => {
  ...
  if (!formData.name || !formData.name.trim()) {
    errors.value.name = '請輸入單位標籤名稱'
    isValid = false
  } else if (formData.name.trim().length < 2) {
    errors.value.name = '單位標籤名稱至少需要2個字符'
    isValid = false
  } else if (formData.name.trim().length > 50) {
    errors.value.name = '單位標籤名稱不能超過50個字符'
    isValid = false
  }
...
const loadTagDetail = async () => {
  ...
  try {
    ...
    const result = await tagStore.fetchTagDetail(route.params.id)
    console.log('載入標籤詳細資料:', result)
    ...
    if (result.success) {
      ...
      console.error('載入標籤詳細資料失敗:', result.message)
      alert(`載入失敗: ${result.message}`)
    }
  } catch (error) {
    console.error('載入標籤詳細資料異常:', error)
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (isEditMode.value) {
      // 編輯模式
      result = await tagStore.updateTag(route.params.id, submitData)
    } else {
      // 新增模式
      result = await tagStore.createTag(submitData)
    }
...
const handleMoveToTop = async () => {
  ...
  try {
    ...
    const result = await tagStore.handleMoveToTop(route.params.id)
...
const deleteTag = async (item) => {
  ...
  try {
    ...
    if (result.success) {
      ...
      // 如果刪除的是當前編輯的標籤，跳轉到新增模式
      if (isEditMode.value && item.id === tagId.value) {
    ...
    <div class="page-container" :class="{ 'mobile-layout': isMobile, 'tablet-layout': isTablet }">
      ...
      <div class="left-section" :class="{ 'mobile-hidden': isMobile && showMobileList }">
        ...
        <div v-if="!isEditMode" class="form-card">
          <h3 class="form-title">新增單位標籤</h3>
          
          <form @submit.prevent="handleSubmit" class="tag-form">
            <div class="form-group">
              <label for="tagName" class="form-label">單位標籤名稱</label>
              <input
                id="tagName"
                type="text"
        ...
        <div v-else class="info-card">
          ...
          <div v-else class="info-content">
            <div class="form-group">
              <label for="editTagName" class="form-label">單位標籤名稱</label>
              <input
                id="editTagName"
                type="text"
                v-model="formData.name"
        ...
        <div class="list-card">
          ...
          <div v-if="isMobile" class="mobile-card-list">
            <div 
              ...
              :key="item.id" 
              class="mobile-card"
              :class="{ 'active': isEditMode && item.id === tagId }"
              @click="editTag(item)"
            >
            ...
            <div v-if="tagData.length === 0" class="mobile-no-data">
              暫無單位標籤資料
            </div>
          </div>
          ...
          <div v-else class="table-container">
            <table class="data-table">
              ...
              <tbody>
                ...
                <tr v-if="tagData.length === 0">
                  <td colspan="4" class="no-data">暫無單位標籤資料</td>
                </tr>
              </tbody>
            </table>
          </div>
