...
const viewTodo = (id) => {
  ...
  router.push(`/settings/view-todo/${id}`)
}
...
const getTodoData = async(searchForm, column = "repair_time", sortDirection = "desc", limit = 10, page = 1) => {
  ...
  try {
    await todoStore.fetchTodos(searchForm, column, sortDirection, limit, page);
    console.log('待辦資料:', todoStore.todos);
    
    if (todoStore.todos && todoStore.todos.data) {
      todoData.value = todoStore.todos.data;
      totalPages.value = todoStore.todos.data.totalPages || 0;
      totalItems.value = todoStore.todos.data.total || 0;
    } else {
      todoData.value = [];
      totalPages.value = 0;
      totalItems.value = 0;
