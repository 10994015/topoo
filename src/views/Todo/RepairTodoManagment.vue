  ...
  <div class="todo-management">
    ...
    <section class="table-section">
      ...
      <div class="table-container" v-if="!isMobile">
        <table class="data-table">
          ...
          <tbody>
            ...
            <!-- 正常資料顯示 -->
            <tr v-else v-for="(item, index) in todoData.data" :key="item.id" :class="['table-row', item.isAlter ? 'alert' : '']">
              <td>{{ index + 1 }}</td>
              <td>{{ item.title }}</td>
      ...
      <div class="mobile-cards" v-else>
        ...
        <!-- 正常資料顯示 -->
        <div v-else v-for="(item, index) in todoData.data" :key="item.id" :class="['mobile-card', item.isAlter ? 'alert' : '']" @click="viewTodo(item.id)">
